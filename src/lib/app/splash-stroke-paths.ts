/** Stage 1 stroke paths — shared production splash + debug tooling. */
export const NUME_SPLASH_STROKE_PATHS = {
  /** Path 1 — opens from bottom-left (1). */
  path1: "M16.13 75 L32.2 25 L39.29 25",
  /** Path 2 — opens from bottom-right. */
  path2: "M30.29 75 L46.37 25 M46.37 25 L39.29 25",
  /** Path 3 — inner diagonal (curtain left edge). */
  path3: "M32.2 25 L16.13 75 L23.21 75",
  /** Path 4 — inner diagonal (curtain right edge). */
  path4: "M46.37 25 L30.29 75 M30.29 75 L23.21 75",
} as const;

export type SplashStrokePathKey = keyof typeof NUME_SPLASH_STROKE_PATHS;

export const NUME_SPLASH_STROKE_ORDER: SplashStrokePathKey[] = [
  "path1",
  "path2",
  "path3",
  "path4",
];

/** Full logo fill paths from brand-flatten (viewBox 0 0 100 100). */
export const NUME_SPLASH_LOGO_FILLS = [
  "M6.44938 77.8931C6.04258 79.1603 6.26759 80.5448 7.0516 81.6203C7.83592 82.696 9.08538 83.3333 10.4167 83.3333H33.3333C35.1433 83.3333 36.7468 82.1634 37.3006 80.4403L56.0506 22.1069C56.4574 20.8398 56.2324 19.4552 55.4484 18.3797C54.6641 17.304 53.4146 16.6667 52.0833 16.6667L29.1667 16.6667C27.3567 16.6667 25.7532 17.8366 25.1994 19.5597L6.44938 77.8931ZM32.2021 25L46.3664 25L30.2938 75H16.1296L32.2021 25Z",
  "M43.9494 77.8931C43.5426 79.1603 43.7676 80.5448 44.5516 81.6203C45.3359 82.696 46.5854 83.3333 47.9167 83.3333H70.8333C72.6433 83.3333 74.2468 82.1634 74.8006 80.4403L93.5506 22.1069C93.9574 20.8398 93.7324 19.4552 92.9484 18.3797C92.1641 17.304 90.9146 16.6667 89.5833 16.6667L66.6667 16.6667C64.8567 16.6667 63.2532 17.8366 62.6994 19.5597L43.9494 77.8931ZM69.7021 25L83.8664 25L67.7938 75H53.6296L69.7021 25Z",
  "M70.8333 79.1667L52.0833 20.8333L29.1667 20.8333L47.9167 79.1667H70.8333Z",
  "M25.8016 18.3797C25.0176 19.4552 24.7926 20.8398 25.1994 22.1069L43.9494 80.4403C44.5032 82.1634 46.1067 83.3333 47.9167 83.3333H70.8333C72.1646 83.3333 73.4141 82.696 74.1984 81.6203C74.9824 80.5448 75.2074 79.1603 74.8006 77.8931L56.0506 19.5597C55.4967 17.8366 53.8933 16.6667 52.0833 16.6667L29.1667 16.6667C27.8354 16.6667 26.5859 17.304 25.8016 18.3797ZM49.0438 25L65.1164 75H50.9521L34.8796 25H49.0438Z",
] as const;

export const NUME_SPLASH_LOGO_SIZE_PX = 140;
export const NUME_SPLASH_VIEWBOX_SIZE = 100;
export const NUME_SPLASH_STROKE_WIDTH_PX = 6;

/** Inner diagonal segments used for curtain edge geometry (path 3 & 4). */
export const NUME_SPLASH_CURTAIN_DIAGONALS = {
  path3: { x1: 32.2, y1: 25, x2: 16.13, y2: 75 },
  path4: { x1: 46.37, y1: 25, x2: 30.29, y2: 75 },
} as const;
